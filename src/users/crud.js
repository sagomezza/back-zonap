        ...
        try {
            ...
            if (parameter.type === "full") {
                ...
                if (parameter.monthlyUser) {
                    ...
                    if (!parameter.mensualityType) { reject({ response: -1, message: `Missing data: mensualityType` }); return }
                }
            } else {
                if (!parameter.phone) { reject({ response: -1, message: `Missing data: phone` }); return }
                Object.assign(parameter, {
                    ...
                    cars: parameter.plate ?  [
                        { plate: parameter.plate, brand: "" }
                    ] : [],
            ...
            if (parameter.phone) {
                ...
                try {
                    if (parameter.plates && parameter.plates.length && parameter.plates.length > 0) hasMensuality = await mensualityCrud.findMensualityPlate({ plate: parameter.plates[0] })
                } catch (err) {
                    //console.log(err)
                }
                const db = admin.firestore();
                let userRef = db.collection('users')
                let query = userRef.where('phone', '==', parameter.phone).get()
                    .then(async (snapshot) => {
                        try {
                            if (snapshot.empty) {
                                if (parameter.isMonthlyUser) {
                                    ...
                                    console.log(data)
                                    mensualityCrud.createMensuality(data)
                                        .then(async (resultMen) => {
                                            resolve({ response: 1, message: `User created succesfully`, id: response.id })
                                            return;
                                        })
                                        .catch(err => {
                                            console.log(err)
                                ...
                                snapshot.forEach(async (doc) => {
                                    ...
                                    user.get()
                                        .then(async (result) => {
                                            try {
                                                if (result.data().plates.includes(parameter.plates[0]) || parameter.plates.length === 0 ) {
                                                    reject({ response: -1, message: "User already exists and there was no new info to update" })
                                                    return
                                                } else {
                                                    await user.update({
                                                        plates: admin.firestore.FieldValue.arrayUnion(parameter.plates[0])
                                                    })
                                                    resolve({ response: 2, message: `User already exists but added the new plate to the profile` })
                                                }
                                            } catch (err) {
                                                console.log(err)
                                                reject(err)
            ...
            let query = userRef.where('phone', '==', parameter.phone).get()
                .then(snapshot => {
                    ...
                    snapshot.forEach(async (doc) => {
                        let data = doc.data()
                        data.id = doc.id
                        resolve({ response: 1, message: `User found succesfully`, data: data })
                    });
                })
                .catch(err => {
...
module.exports.findUserByPlate = (parameter) => {
    return new Promise((resolve, reject) => {
        ...
        const db = admin.firestore();
        let userRef = db.collection('users')
        let query = userRef.where('plates', "array-contains", parameter.plate).get()
            .then(snapshot => {
                if (snapshot.empty) {
                    reject({ response: -1, message: `User not found` })
                    return;
                }
                blCrud.blackListForPlate({ plate: parameter.plate })
                    .then(blRes => {
                        let users = []
                        let fullData = []
                        ...
                        if (err.response && err.response === -1) {
                            ...
                            resolve({ response: 1, message: `User found succesfully`, data: users, fullData })
                        } else reject(err)
                    })
            })
            .catch(err => {
...
module.exports.changeUserPhoneNumber = (parameter) => {
    return new Promise((resolve, reject) => {
        if (Object.values(parameter).length === 0) { reject({ response: -1, message: `Error: Empty object` }); return }
        if (!parameter.uid) { reject({ response: -1, message: `Missing data: uid` }); return }
