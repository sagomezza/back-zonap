                  ...
                  if (!currentReserve.mensuality) {
                    ...
                    if (coupon) {
                      if (currentReserve.type === "car") {
                        ...
                        console.log(
                          total,
                          parseFloat(coupon.value.car.hours) / 100, Math.round(
...
module.exports.getTotalWithCoupon = (parameter) => {
  return new Promise((resolve, reject) => {
    try {
      if (Object.values(parameter).length === 0) {
        reject({ response: -1, message: `Error: Empty object` });
      ...
      if (!parameter.vehicleType) {
        reject({ response: -1, message: `Missing data: hqId` });
        return;
      }
      if (!parameter.parkingType) {
        reject({ response: -1, message: `Missing data: phone` });
        return;
      }
          ...
          coupons
            ...
            .then(async (coupons) => {
              ...
              if (coupons.response === 1) {
                ...
                if(parameter.parkingType === 'monthly'){
                  if (parameter.vehicleType === 'car'){
                    total = doc.data().monthlyCarPrice;
                    ...
                    total = doc.data().monthlyBikePrice;
